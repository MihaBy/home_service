(function($, undefined){
  ...
  //end test
    //hide modal
      ...
      //add class pos fixed add display flex add scroll-body
      // remove after close display none position auto?
      // reinit slick offset 
      // get data-skick-el = [0],
      // get p_extend
      // get h_extend
      // insert data in modal
      // add new slider synhonize with main slider, !!!
      //slickGoTo for prev nav block + currentSlide

 /*
  $('[data-skick="services__left__arr"]').on('click', function(event) {
    console.log("slider__left");
    $('[data-skick="services"]').slick('slickPrev');
  })
 */
    // modal show video
    const modalvideo = {
    ...
    //add soft scroll to elements
    //reviews slider
/*
    const reviews_slider2 = {
      sliderId:'[data-skick="reviews"]',
      sliderElClass:'reviews-slide',
      sliderElSubClass:{
        0 :'reviews-slide__left-col',
        1 :'reviews-slide__right-col'},
      sliderElContenClass:{
        0 :'reviews-slide__content',
        1 :'reviews-slide__autor'},
      prevArrow: '<button type="button" class="reviews-prev">Previous<svg><use xlink:href="'+ serv_slider_root_path_arows +'/'+ serv_slider_src_left_arow +'"></use></svg></button>',
      nextArrow: '<button type="button" class="reviews-next">Next<svg><use xlink:href="'+ serv_slider_root_path_arows +'/'+ serv_slider_src_right_arow +'"></use></svg></button>',
      sliderContent:[
        {
          src:'https://placehold.co/654x654',
          text:'I always get a friendly, personal reception from both Mary Jane and his staff. When I needed multiple services but had maxed out my insurance, Mary Jane worked with me to identify the worst problem and work out a plan for the rest. Excellent, caring work. I highly recommend Mary Jane and his team.',
          autor:' Aurora J.',
        },
        {
          src:'https://placehold.co/654x654',
    ...
    //console.log(JSON.stringify(reviews_slider2))
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      //let countReviewsSliderEl = 0;
        
        for(let key in reviews_slider.sliderContent) {
          // if(reviews_slider.sliderContent.hasOwnProperty(key)) {
          //   console.log(key);
          //   console.log('key');
          //   countReviewsSliderEl++;
          // }
